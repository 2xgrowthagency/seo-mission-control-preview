...
repository remains private. The initial dashboard uses clearly labelled
synthetic data, not current client metrics.

## File-backed data

The app fetches `data/portfolio.example.json` on page load and when **Reload
source** is clicked. It is a schema-validated JSON snapshot; no database or
...
between people. Shared changes go through the snapshot file. Real portfolio
data requires an approved public field scope before publication.
